...
function tinyStopAnim(kto)
	if kto then
		...
		local query = exports["aj-dbcon"]:upd("INSERT INTO server_jobslog SET joblog_code=?, joblog_data=NOW(), joblog_value=1, joblog_userid=?, joblog_type=?",code,uid,jtype);
		...
		if gemmolog then
			...
			if jobCode == "gemmolog-sa" then
				...
				if isElement(plr) and getElementType(plr) == "player" then
					...
					if uid > 0 then
						local query = exports["aj-dbcon"]:upd("UPDATE server_users SET user_money=user_money+'"..kwota.."'  WHERE user_id = '"..uid.."'");
						local query = exports["aj-dbcon"]:upd("UPDATE server_users SET user_bankmoney=user_bankmoney+"..kwota.." WHERE user_id = '22'");
						local query = exports["aj-dbcon"]:upd("INSERT INTO server_logibankowe SET lbank_userid = '22', lbank_touserid = '"..uid.."', lbank_kwota='"..kwota.."', lbank_data=NOW(), lbank_desc='Praca dorywcza gemmolog.', lbank_type='1'");
					end
...
function kamien(code,uid,type)
	if type and uid and code then
		local mat = exports["aj-dbcon"]:wyb("SELECT * FROM server_jobslog WHERE joblog_code=? AND joblog_userid=? AND joblog_value=1 AND joblog_type=?",code,uid,type);
		local mat = #mat;
		return mat
	end
	return false
	...
	if isElement(plr) then
		...
		if uid > 0 then
			...
			if mat > 0 then
				...
				local query = exports["aj-dbcon"]:upd("INSERT INTO server_logibankowe SET lbank_userid = '22', lbank_touserid = '"..uid.."', lbank_kwota='"..calycost.."', lbank_data=NOW(), lbank_desc='Sprzedaż surowców (jubiler).', lbank_type='1'");
				
				local query = exports["aj-dbcon"]:upd("UPDATE server_jobslog SET joblog_value=0 WHERE joblog_code=? AND joblog_userid=? AND joblog_value=1 AND joblog_type=?",code,uid,type);
				refreshdane(plr,uid)
			end
		end
	end
