...
local pojazdy = {
	...
	{0, 407, "siren-strom-1.wav", "babyshark.wav", "hespirates.wav", "traby.ogg"}, -- PSP
	...
	{136, 0, "policja1.ogg", "syrenasapd.ogg", 0, "sapdhorn.wav"}, -- SAPD Dc255.ogg
	{141, 0, "policja1.ogg", "syrenasapd.ogg", 0, "sapdhorn.wav"}, -- SAPD
	{151, 0, "policja1.ogg", "syrenasapd.ogg", 0, "sapdhorn.wav"}, -- SAP
	
	--{108, 0, "siren-strom-1.wav", "babyshark.wav", "hespirates.wav", "ultrakox.wav"}, -- test
}
...
--[[ zapis v 1.0 (buguje sie, nie poelcam, w 2.09 znaprawione)		
		local sp1 = sprSiren(source,3,veh_id) or 0;
		local sp2 = sprSiren(source,4,veh_id) or 0;
		local test = sprSiren(source,5,veh_id) or 0;
		local sp4 = sprSiren(source,6,veh_id) or 0;
		--unbind w raazie co
		if sp1 ~= 0 then unbindKey("1", "down", sirensTest) end
		if sp2 ~= 0 then unbindKey("2", "down", sirensTest) end
		if sp3 ~= 0 then unbindKey("3", "down", sirensTest) end
		if sp4 ~= 0 then unbindKey("r", "down", sirensTest) end
		-- bind nowe
		if sp1 ~= 0 then bindKey("1", "down", sirensTest, 3, 0) end
		if sp2 ~= 0 then bindKey("2", "down", sirensTest, 4, 0) end
		if sp3 ~= 0 then bindKey("3", "down", sirensTest, 5, 0) end
		if sp4 ~= 0 then bindKey("r", "down", sirensTest, 0, 1) end
]]--	 ver 2.0 (odbugowane)
