				...
				if not getPlayerTeam( player ) or not (showTeams or (serverInfo.forceshowteams and not serverInfo.forcehideteams)) or serverInfo.forcehideteams then
					...
					for key, column in ipairs( scoreboardColumns ) do
						...
						if column.name == "name" then
							...
							local character=getElementData(player,"player:dbid")
...
--							table.insert( colorCodes, { { 255,255,255 }, 1 } )
							...
							local mechanik=getElementData(player,"player:duty")
							local jobColor=getElementData(player,"player:jobCOLOR") or false;
							--local zamaskowany=getElementData(player,"player:zamaskowany")
							if (not character) then
							    table.insert( colorCodes, {{100,100,100},1})
							elseif (jobColor) then
							    table.insert( colorCodes, {{jobColor[1],jobColor[2],jobColor[3]},1})
							elseif (mechanik) then
								table.insert( colorCodes, {{86, 129, 200},1})						
							elseif (premium) then
							    table.insert( colorCodes, {{239, 208, 12},1})						
							else
							    table.insert( colorCodes, {{255,255,255},1})						
							end
...
--							end
						elseif column.name == "player:frakcjaCODE" then
							local dutyon = getElementData(player,"player:frakcja") or false;
							if dutyon then
								--noti
								local dcode = getElementData(player,"player:frakcjaCODE") or "";
								content = dcode;
							else
								content = "";
							end
...
--			if serverInfo.server and showServerInfo then
            ...
            -- mozna dodac logo
			--dxDrawImage( topX-s(10), y-25, 200, 31, "logo_sevr.png", 0,0,0, cWhite, drawOverGUI )

			dxDrawText( "http://ajoneg.ts3dns.eu/forum/", topX+s(5), y+s(1), topX+scoreboardDimensions.width-s(10), y+dxGetFontHeight( fontscale(serverInfoFont, scoreboardScale), serverInfoFont ), cServerInfo, fontscale(serverInfoFont, s(0.75)), serverInfoFont, "left", "top", false, false, drawOverGUI )
...
--[[
			...
			local textLength = dxGetTextWidth( "Wciśnij TAB aby zamknąć", fontscale(rmbFont, s(0.75)), rmbFont )
			local textHeight = dxGetFontHeight( fontscale(rmbFont, s(0.75)), rmbFont )
			dxDrawText( "Wciśnij TAB aby zamknąć", sX/2-(textLength/2), topY+scoreboardDimensions.height-textHeight-s(2), sX/2+(textLength/2), topY+scoreboardDimensions.height-s(2), cWhite, fontscale(serverInfoFont, s(0.75)), rmbFont, "left", "top", false, false, drawOverGUI )
