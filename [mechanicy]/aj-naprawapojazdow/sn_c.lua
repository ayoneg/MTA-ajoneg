...
w_m.grid_koszt = guiGridListAddColumn ( w_m.grid, "Koszt naprawy", 0.3 )
        ...
        w_m.wnd = guiCreateWindow(0.25, 0.25, 0.51, 0.50, "Panel mechanika", true)

        w_m.grid = guiCreateGridList(0.02, 0.07, 0.96, 0.77, true, w_m.wnd)
        guiGridListAddColumn(w_m.grid, "Nazwa części", 0.35)
        guiGridListAddColumn(w_m.grid, "Stan części", 0.3)
        guiGridListAddColumn(w_m.grid, "Koszt naprawy", 0.3)
        ...
        w_m.lbl_costinfo = guiCreateLabel(0.71, 0.87, 0.26, 0.10, "", true, w_m.wnd)
        guiLabelSetHorizontalAlign(w_m.lbl_costinfo, "center", false)
        guiLabelSetVerticalAlign(w_m.lbl_costinfo, "center")    
...
addEventHandler("onClientMarkerHit", resourceRoot, function(el,md)
	...
	local owner = getElementData(pojazdy[1],"vehicle:owner") or 0;
	local uidplr = getElementData(el,"player:dbid") or 0;
	if owner == uidplr then
	    outputChatBox("Nie możesz naprawić pojazdu, którego jesteś właścicielem.")
	return end
