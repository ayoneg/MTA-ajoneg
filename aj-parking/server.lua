...
local wyjmij_punkt = {
	{"przecho-lv", 1985.7275390625, 2487.912109375, 10.843439102173}, -- LV
	{"przecho-fc", 50.58203125, 1163.810546875, 18.6640625}, -- test FC
}

local parkingi = {
	{1976.205078125, 2441.380859375, 10.8, 36.5, 47.5, 5.0}, -- LV
	{40.7802734375, 1153.783203125, 18.6640625, 35, 27, 5.0}, -- test FC
}

local resp_poj = {
	-- przecho LV
	{"przecho-lv", 1979.0927734375, 2477.162109375, 10.837408065796, 0, 10, 5, -90},
	{"przecho-lv", 1979.0927734375, 2470.162109375, 10.837408065796, 0, 10, 5, -90},
	{"przecho-lv", 1979.0927734375, 2463.162109375, 10.837408065796, 0, 10, 5, -90},
	{"przecho-lv", 1979.0927734375, 2456.162109375, 10.837408065796, 0, 10, 5, -90},
	{"przecho-lv", 1979.0927734375, 2449.162109375, 10.837408065796, 0, 10, 5, -90},	
	{"przecho-lv", 2005.109375, 2484.162109375, 10.837408065796, 0, 10, 5, 90}, -- 2 strona
	{"przecho-lv", 2005.109375, 2477.162109375, 10.837408065796, 0, 10, 5, 90}, 
	{"przecho-lv", 2005.109375, 2470.162109375, 10.837408065796, 0, 10, 5, 90},
	{"przecho-lv", 2005.109375, 2463.162109375, 10.837408065796, 0, 10, 5, 90},
	{"przecho-lv", 2005.109375, 2456.162109375, 10.837408065796, 0, 10, 5, 90},
	{"przecho-lv", 2005.109375, 2449.162109375, 10.837408065796, 0, 10, 5, 90},

	-- przecho FC
	{"przecho-fc", 59.0986328125, 1159.3486328125, 18.6640625, 0, 5, 10, 0},
	{"przecho-fc", 66.0986328125, 1159.3486328125, 18.6640625, 0, 5, 10, 0},
	{"przecho-fc", 73.0986328125, 1159.3486328125, 18.6640625, 0, 5, 10, 0},
}

for i,v in ipairs(wyjmij_punkt) do
    visumark = createMarker(v[2], v[3], v[4]-1, "cylinder", 2, 255,255,255,50)
	prz = createColSphere(v[2], v[3], v[4], 1)
	blip = createBlipAttachedTo(prz, 35, 2, 0, 0, 0, 0, -100, 800);
	setElementData(prz,"przecho:wyjmij",true);
	setElementData(prz,"przecho:code",v[1]);
end

for i,v in ipairs(parkingi) do
	obszar = createColCuboid(v[1], v[2], v[3]-1, v[4], v[5], v[6]);
	setElementData(obszar,"przecho:obszarparkin",true);
end

for i,v in ipairs(resp_poj) do
	obszar = createColCuboid(v[2]-2.5, v[3]-5, v[4]-1, v[6], v[7], 5);
	setElementData(obszar,"przecho:respobszar",v[1]);
	setElementData(obszar,"przecho:respobszarRXR",v[8]);
	setElementData(obszar,"przecho:respobszarX1",v[6]);
	setElementData(obszar,"przecho:respobszarY1",v[7]);
--	setElementRotation(obszar, 0, 0, v[8])
end

function sprObszary(cubID)
    cuboid=getElementsByType('colshape');
    for i,var in pairs(cuboid) do
		local cubboidID = getElementData(var,"przecho:respobszar") or false;
		if cubboidID ~= false and cubboidID == cubID and #getElementsWithinColShape(var,"vehicle") == 0 then
			return var
		end
	end
end
...
addEventHandler("GetRec", root, function(vart)
	...
	if tonumber(vart) == 1 and tonumber(plr_orgID) > 0 then 
		local result = exports["aj-dbcon"]:wyb("SELECT * FROM server_vehicles WHERE veh_groupowner='"..plr_orgID.."' AND veh_parking='1'");
    	if #result > 0 then
        	triggerClientEvent(source, "GetVehicles", source, result)
    	end
	end
	local sluzba = getElementData(source,"player:frakcja") or false
	if tonumber(vart) == 2 and sluzba then 
		local code = getElementData(source,"player:frakcjaCODE") or false
		local result = exports["aj-dbcon"]:wyb("SELECT * FROM server_vehicles WHERE veh_frakcja='"..tostring(code).."' AND veh_parking='1'");
    	if #result > 0 then
        	triggerClientEvent(source, "GetVehicles", source, result)
    	end
	end
...
addEventHandler("onVehicleExit", root, function(plr,seat)
	...
	local engState = getElementData(source,"vehicle:engineState")
	if engState then
...
addEventHandler("onVehicleStartEnter", root, function(plr,seat)
	if plr and source then 
	    if seat == 0 then
			...
			if (KatB[model] and playerKatB == 0) or (KatC[model] and playerKatC == 0) or (KatS[model] and playerKatS == 0) then 
			...
			-- osobno kat A
			local veh_data = getElementData(source,"vehicle:job") or false;
			if (KatA[model] and playerKatA == 0 and veh_data ~= "FastEats") then 
				outputChatBox("* Nie masz prawa jazdy do prowadzenia tego pojazdu.", plr,255,0,0)
				cancelEvent() 
				return 
			end
			...
			-- frakcje
			if getElementData(source,"vehicle:owner")==22 and getElementData(source,"vehicle:frakcjaCODE") == getElementData(plr,"player:frakcjaCODE") and getElementData(plr,"player:frakcja") then return end
...
addEvent("VehicleSpawnPrzechoV2", true)
addEventHandler("VehicleSpawnPrzechoV2", root, function(id,model,plr)
	local przechoCODE = getElementData(plr,"player:przechoCODE") or false;
	if przechoCODE ~= false then
		if id > 0 then
			local getThisShit = sprObszary(przechoCODE) or false;
			if getThisShit ~= false then
				local x,y,z = getElementPosition(getThisShit)
				local RXR = getElementData(getThisShit,"przecho:respobszarRXR") or 0;
				--setElementPosition(plr,x+2.5,y+5,z+1)
				local rX1 = getElementData(getThisShit,"przecho:respobszarX1") or 0;
				local rY1 = getElementData(getThisShit,"przecho:respobszarY1") or 0;
				
				local new_rX1 = rX1/2;
				local new_rY1 = rY1/2;
				
				local x,y,z = x+new_rX1,y+new_rY1,z+1;
				...
				exports["aj-vehs"]:respPojazd(source,carid,value,x,y,z,RXR)
			end
		end
	end
end)
...
if not md or not veh then return end
local typeEle = getElementType(veh) or false;
    if typeEle == "vehicle" then
	local sphareCODE = getElementData(source,"przecho:obszarparkin") or false
	if sphareCODE ~= false then
...
addEventHandler("onColShapeHit", root, shapeHit)  
...
addEventHandler("onColShapeLeave", root, shapeLeave) 
